...
class SellController extends ApiController
{
     ...
     * Get the specified sell
     *
     * @urlParam sell required invoice no  Example: 55 or ss4w
     *
     * @response {
        "data": [
            {
     ...
     *
     * @bodyParam transaction_id integer required Id of the sell
     * @bodyParam amount float discount amount applicable to the sale Example: 10.0000
     *
     * @response {
        "id": 159,
        "business_id": 1,
     ...
     */
    public function addSellPayment(Request $request)
    {
        try {
            $input = $request->except('_token');
            ...
            $business_id = $user->business_id;
            if (!empty($input['id']) && !empty($input['amount'])) {
                DB::beginTransaction();

                $payment_amount = $this->transactionUtil->num_uf($input['amount']);
                ...
                if ($payment_amount != 0) {
                    ...
                    if ($transaction->payment_status == 'paid') {
                        return $this->otherExceptions('already paid');
                    }
                    if ($transaction->payment_status == 'partial') {
                        $amount = TransactionPayment::where('transaction_id', $input['id'])->sum('amount');
                    } else {
                        $amount = 0;
                    }
                    if ($input['amount'] > $transaction->final_total - $amount) {
                        return $this->otherExceptions('the amount should be less or equal the invoice total');
                    }
                    $payment_data = [
                    ...
                    if ($payment_amount == $transaction->final_total - $amount) {
                    ...
                    $output = $this->respond([
                        'success' => 1,
                        'msg' => trans('lang_v1.payment_account_success'),
                    ]);
                } else {
                    return $this->otherExceptions('the amount cannot be 0');
                }

                DB::commit();
            } else {
                return $this->otherExceptions('the amount cannot be empty');
            }
        } catch (ModelNotFoundException $e) {
            DB::rollback();
            $output = $this->modelNotFoundExceptionResult($e);
