                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules, $is_admin, $pos_settings) {
                        ...
                        if (auth()->user()->can('smart_root_discount.access')) {
                            $sub->url(
                                action([\App\Http\Controllers\DiscountController::class, 'smart_root_discounts']),
                                __('lang_v1.root_smart_discounts'),
                                ['icon' => '', 'active' => request()->segment(1) == 'smart_root_discounts']
                            );
                        }
                    ...
                    function ($sub) use ($enabled_modules, $is_admin) {
                        ...
                        if (auth()->user()->can('contacts_report.view')) {
                         ...
                         if (auth()->user()->can('user_contacts_due_report.access')) {
                            $sub->url(
                                action([\App\Http\Controllers\ReportController::class, 'getUsersContactsDueReport']),
