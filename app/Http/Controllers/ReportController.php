...
class ReportController extends Controller
{
            ...
            return Datatables::of($contacts)
                ...
                ->editColumn(
                 ...
                 ->editColumn('total_purchase', function ($row) {
                    $formatted = $this->transactionUtil->num_f($row->total_purchase, true);
                    return '<span class="total_purchase" data-orig-value="' . $row->total_purchase . '">' . $formatted . '</span>';
                })
                ->editColumn('total_invoice', function ($row) {
                    $formatted = $this->transactionUtil->num_f($row->total_invoice, true);
                    return '<span class="total_invoice" data-orig-value="' . $row->total_invoice . '">' . $formatted . '</span>';
                })
              ...
              ->addColumn('opening_balance_due', function ($row) {
                    $value = $row->opening_balance - $row->opening_balance_paid;
                    $formatted = $this->transactionUtil->num_f($value, true); // Format with currency symbol
                    return '<span class="opening_balance_due" data-orig-value="' . $value . '">' . $formatted . '</span>';
                })
                ->addColumn('last_payment', function ($row) {
                        // Get the last payment for this contact
                        $last_payment = TransactionPayment::whereHas('transaction', function ($q) use ($row) {
                            $q->where('contact_id', $row->id);
                        })
                        ->orderBy('paid_on', 'desc')
                        ->first();

                        return $last_payment ? $last_payment->payment_ref_no : '<span class="text-muted">—</span>';
                    })
                ->addColumn('last_payment_date', function ($row) {
                        ...
                        ->orderBy('paid_on', 'desc')
                        ->first();
                        return $last_payment ? $last_payment->paid_on : '<span class="text-muted">—</span>';
                    })
                 ->addColumn('days_count_from_last_payment', function ($row) {
                    ...
                    if ($last_payment && $last_payment->paid_on) {
                        $paidOn = \Carbon\Carbon::parse($last_payment->paid_on);
                        $now = \Carbon\Carbon::now();
                        $daysDiff = $paidOn->diffInDays($now);
                ...
                ->addColumn('last_transaction', function ($row) {
                        $last_transaction = Transaction::where('contact_id', $row->id)
                        ->orderBy('transaction_date', 'desc')
                        ->first();
                        return $last_transaction ? $last_transaction->transaction_date : '<span class="text-muted">—</span>';
                    })
                ...
                ->rawColumns(['total_purchase', 'total_invoice', 'due', 'name', 'total_purchase_return', 'total_sell_return', 'opening_balance_due','last_payment','last_payment_date','days_count_from_last_payment','last_transaction'])
                ->make(true);
        }
 ...
 public function getUsersContactsDueReport(Request $request)
{
    if (!auth()->user()->can('user_contacts_due_report.access')) {
        abort(403, 'Unauthorized action.');
    }
   ...
   $users = User::where('business_id', $business_id)
    ...
    ->pluck('full_name', 'id')
    ->toArray();
    $usersFilter = User::where('business_id', $business_id)
       ->where('status', 'active')
    ->select(DB::raw("CONCAT(first_name, ' ', last_name) AS full_name"), 'id')
    ->pluck('full_name', 'id')
    ->toArray();
    ...
    if ($request->ajax()) {
        ...
        if (!empty($contact_id)) {
            $contactsQuery->where('id', $contact_id);
        }
        ...
        foreach ($contacts as $contact) {
            $row = [
                'contact_name' => $this->contactNameWithLink($contact),
            ];

            $total_due = 0;

            foreach ($users as $user_id => $username) {
                $due = $this->calculateContactUserDue($contact->id, $user_id, $business_id, $start_date, $end_date,$user_filter ,$location_id);
                
                // Store both raw and formatted values like in getCustomerSuppliers
                $row["user_".$user_id] = $due;
                $row["user_".$user_id."_display"] = '<span class="user_due" data-orig-value="'.$due.'">'.$this->transactionUtil->num_f($due, true).'</span>';
                
                $total_due += $due;
            }

            $row['total'] = $total_due;
            $row['total_display'] = '<span class="total_due" data-orig-value="'.$total_due.'">'.$this->transactionUtil->num_f($total_due, true).'</span>';
            $data[] = $row;
        }
    ...
    $business_locations = BusinessLocation::forDropdown($business_id, true);
    $contact_dropdown = Contact::where('type','customer')
                                ->pluck('name', 'id')
                                ->toArray();

    return view('report.users_contacts_due_report', compact('usersFilter','types','business_locations','contact_dropdown','users'));
}

  private function contactNameWithLink($contact)
{
    $name = $contact->name;
    ...
    return '<a href="' . action([\App\Http\Controllers\ContactController::class, 'show'], [$contact->id]) . '" target="_blank">' . $name . '</a>';
}

private function calculateContactUserDue(
    $contact_id,
    $user_id,
    $business_id,
    $start_date = null,
    ...
    if (!empty($user_filter)) {
        $query->where('t.created_by', $user_filter);
    }
  ...
  if (!$result) return 0.00;
    ...
    $due = ($total_invoice - $invoice_received)
         - ($total_purchase - $purchase_paid)
         - ($total_sell_return - $sell_return_paid)
         + ($total_purchase_return - $purchase_return_received)
         + ($opening_balance - $opening_balance_paid);
   
    // Ensure zero is returned as 0, not null or empty
    return $due === null ? 0.00 : $due;
}
    ...
    public function getTotalPaymentsTransaction(Request $request){
        if (! auth()->user()->can('sales_representative.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        if ($request->ajax()) {
            ...
            $transaction_payments = $this->transactionUtil->getTotalPaymentsTransaction($business_id, $start_date, $end_date, $created_by);

            return [
                'total_payments' => $transaction_payments['total_payments'],
            ];
        }
    }
      ...
      public function getTotalDue(Request $request){
        ...
        if ($request->ajax()) {
            ...
            $total_due = $this->transactionUtil->getTotalDue($business_id, $start_date, $end_date,$location_id, $created_by);

            return [
                'total_due' => $total_due['total_due'],
            ];
        }
    }
