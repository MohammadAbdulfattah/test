    ...
    */
    ...
    public function postRegister(Request $request)
    {
        ...
        try {
            ...
            $business_details['enabled_purchases_modules'] = ['purchases', 'add_sale', 'pos_sale', 'stock_transfers', 'stock_adjustment', 'expenses'];
            $business_details['enabled_sale_modules'] = ['enable_sale_note', 'enable_discount', 'enable_document_attachment', 'enable_invoice_number', 'enable_sale_date'];
            $business_details['enabled_products_modules'] = ['enable_sku_symbol', 'enable_stock_management', 'enable_description', 'enable_product_image', 'enable_type', 'enable_brand', 'enable_categories'];
            $business_details['enabled_purchases_modules'] = ['enable_shipping', 'enable_purchase_note', 'enable_tax', 'enable_discount', 'enable_document_attachment', 'enable_purchase_date', 'enable_reference_number', 'enable_sale_period'];
            $business_details['enabled_quick_list'] = ['enable_add_user','enable_add_product','enable_add_purchase'];
    ...
    public function getBusinessSettings()
    {
        ...
        $products_modules = $this->moduleUtil->availableProductsModules();
        $sale_modules = $this->moduleUtil->availableSaleModules();
        $purchases_modules = $this->moduleUtil->availablePurchasesModules();
        $user_modules = $this->moduleUtil->availableUserModules();
        $expenses_modules = $this->moduleUtil->availableExpensesModules();
        $contacts_modules = $this->moduleUtil->availableContactsModules();
        $quick_list = $this->moduleUtil->availableQuickList();
        ...
        return view('business.settings', compact('is_gbs','business', 'quick_list','currencies', 'tax_rates', 'timezone_list', 'months', 'accounting_methods', 'commission_agent_dropdown', 'units_dropdown', 'date_formats', 'shortcuts', 'pos_settings', 'modules', 'sale_modules', 'purchases_modules', 'products_modules', 'user_modules', 'contacts_modules', 'expenses_modules','theme_colors', 'email_settings', 'sms_settings', 'mail_drivers', 'allow_superadmin_email_settings', 'custom_labels', 'common_settings', 'weighing_scale_setting', 'payment_types'));
    ...
    public function postBusinessSettings(Request $request)
    {
        ...
        try {
            ...
            if ($request->has('allowed_distance') ) {
                $business_details['allowed_distance'] = $request->input('allowed_distance');
            } 
            ...
            if (! empty($request->input('enable_automatic_correction')) && $request->input('enable_automatic_correction') == 1) {
                $business_details['enable_automatic_correction'] = 1;
            } else {
                $business_details['enable_automatic_correction'] = 0;
            }
            ...
            $enabled_products_modules= $request->input('enabled_products_modules');
            $enabled_sale_modules= $request->input('enabled_sale_modules');
            $enabled_purchases_modules= $request->input('enabled_purchases_modules');
            $enabled_user_modules= $request->input('enabled_user_modules');
            $enabled_expenses_modules= $request->input('enabled_expenses_modules');
            $enabled_contacts_modules= $request->input('enabled_contacts_modules');
            $enabled_quick_list= !empty($request->input('enabled_quick_list')) ? $request->input('enabled_quick_list'):[];
            if (count($enabled_quick_list) > 12) {
                $output = [
                    'success' => 0,
                    'msg' => __('lang_v1.cannot_enable_more_than_12'),
                ];
            ...
            $business_details['enabled_products_modules'] = ! empty($enabled_products_modules) ? $enabled_products_modules : null;
            $business_details['enabled_sale_modules'] = ! empty($enabled_sale_modules) ? $enabled_sale_modules : null;
            $business_details['enabled_purchases_modules'] = ! empty($enabled_purchases_modules) ? $enabled_purchases_modules : null;
            $business_details['enabled_user_modules'] = ! empty($enabled_user_modules) ? $enabled_user_modules : null;
            $business_details['enabled_expenses_modules'] = ! empty($enabled_expenses_modules) ? $enabled_expenses_modules : null;
            $business_details['enabled_contacts_modules'] = ! empty($enabled_contacts_modules) ? $enabled_contacts_modules : null;
            $business_details['enabled_quick_list'] = ! empty($enabled_quick_list) ? $enabled_quick_list : null;
