...
class ProductUtil extends Util
{
     ...
     * @return bool
     */
    public function updateProductQuantity($location_id, $product_id, $variation_id, $new_quantity, $old_quantity = 0, $number_format = null, $uf_data = true,$van_id=null)
    {
        if ($uf_data) {
        ...
        if($van_id){
            ...
            if ($product->enable_stock == 1 && $qty_difference != 0) {
                ...
                $variation_location_d = VanStock::where('variation_id', $variation->id)
                    ->where('product_id', $product_id)
                ...
                $variation_location_d1 = VariationLocationDetails::where('variation_id', $variation->id)
                    ->where('product_id', $product_id)
                    ->where('product_variation_id', $variation->product_variation_id)
                    ->where('location_id', $location_id)
                    ->first();

                if (empty($variation_location_d1)) {
                    $variation_location_d1 = new VariationLocationDetails();
                    $variation_location_d1->variation_id = $variation->id;
                    $variation_location_d1->product_id = $product_id;
                    $variation_location_d1->location_id = $location_id;
                    $variation_location_d1->product_variation_id = $variation->product_variation_id;
                    $variation_location_d1->qty_available = 0;
                }

                $variation_location_d1->qty_available += $qty_difference;
                $variation_location_d1->qty_in_vans += $qty_difference;
                $variation_location_d1->save();
            }
        }else{
            //Check if stock is enabled or not.
            if ($product->enable_stock == 1 && $qty_difference != 0) {
     ...
     * @return bool
     */
    public function decreaseProductQuantity($product_id, $variation_id, $location_id, $new_quantity, $old_quantity = 0,$van_id=null,$for_van_stock=false)
    {
        $qty_difference = $new_quantity - $old_quantity;
        ...
        if ($product->enable_stock == 1) {
            ...
            if ($van_id) {
                ...
                $details->increment('qty_in_vans', $qty_difference);
            }else{
                $details = VariationLocationDetails::where('variation_id', $variation_id)
                    ->where('product_id', $product_id)
                    ->where('location_id', $location_id)
     ...
     * @return void
     */
    public function decreaseProductQuantityCombo($combo_details, $location_id, $van_id=null)
    {
        if($van_id)
        {
     ...
     * @return array
     */
    public function getDetailsFromVariation($variation_id, $business_id, $location_id = null, $check_qty = true,$van_id=null,$for_van=false)
    {
        ...
        if($van_id){
            ...
            if ($check_qty) {
                $query->where(function ($query) {
                    $query->where('p.enable_stock', '!=', 1)
                        ->orWhere('vs.qty_available', '>', 0);
                });
            }

            if (! empty($van_id) && $check_qty) {
                //Check for enable stock, if enabled check for location id.
                $query->where(function ($query) use ($van_id) {
                    $query->where('p.enable_stock', '!=', 1)
                        ->orWhere('vs.van_id', $van_id);
                });
            }
            ...
            if ($product->product_type == 'combo') {
                if ($check_qty) {
                    $product->qty_available = $this->calculateComboQuantity($location_id, $product->combo_variations,$van_id);
                }

                $product->combo_products = $this->calculateComboDetails($location_id, $product->combo_variations,$van_id);
            }
     ...
     * @return int
     */
    public function calculateComboQuantity($location_id, $combo_variations,$van_id=null)
    {
        //get stock of the items and calcuate accordingly.
        $combo_qty = 0;
        foreach ($combo_variations as $key => $value) {
            if($van_id){
                $variation = Variation::with(['product', 'van_stock' => function ($q) use ($van_id) {
                    $q->where('van_id', $van_id);
                }])->findOrFail($value['variation_id']);
                $vld = $variation->van_stock
                    ->first();
            }else{
     ...
     * @return int
     */
    public function calculateComboDetails($location_id, $combo_variations,$van_id=null)
    {
        $details = [];
     ...
     * @return void
     */
    public function adjustProductStockForInvoice($status_before, $transaction, $input, $uf_data = true,$van_id=null)
    {
        ...
        if ($status_before == 'final' && $transaction->status == 'draft') {
            foreach ($input['products'] as $product) {
                if (! empty($product['transaction_sell_lines_id'])) {
                    if($van_id){
                        ...
                        $this->updateProductQuantity($input['location_id'], $product['product_id'], $product['variation_id'], $product['quantity'], 0, null, false,$van_id);
                        if (isset($product['product_type']) && $product['product_type'] == 'combo') {
                            ...
                            foreach ($product['combo'] as $value) {
                                $this->updateProductQuantity($input['location_id'], $value['product_id'], $value['variation_id'], $value['quantity'], 0, null, false,$van_id);
                            }
            ...
            foreach ($input['products'] as $product) {
                ...
                if($van_id){
                    ...
                    if (isset($product['product_type']) && $product['product_type'] == 'combo') {
                        $this->decreaseProductQuantityCombo($product['combo'], $input['location_id'],$van_id);
    ...
    public function filterProduct(
        ...
        if (!empty($userCategories)) {
            $query->where(function($q) use ($userCategories) {
                $q->whereIn('products.category_id', $userCategories)
                  ->orWhereIn('products.sub_category_id', $userCategories);
            });
        }
    
        // Add user brand filter
        if (!empty($userBrands)) {
            $query->whereIn('products.brand_id', $userBrands);
        }
    ...
    public function getVariationStockDetails($business_id, $variation_id, $location_id,$with_van=false)
    {
       if($with_van){
            $purchase_details = Variation::join('products as p', 'p.id', '=', 'variations.product_id')
                ...
                ->where(function ($q) {
                    $q->whereNotIn('t.type', ['purchase_transfer', 'sell_transfer'])
                        ->orWhere(function ($q2) {
                            $q2->whereIn('t.type', ['purchase_transfer', 'sell_transfer'])
                                ->whereNull('t.van_id');
                        });
                })
                //->where('t.status', 'received')
    ...
    public function getVariationStockHistory($business_id, $variation_id, $location_id,$with_van=false)
    {
        if($with_van){
            $stock_history = Transaction::leftjoin(
                ...
                ->leftjoin('vans as v','transactions.van_id','v.id')
                ->where('transactions.location_id', $location_id)
                ->where(function ($q) {
                    $q->whereNotIn('transactions.type', ['purchase_transfer', 'sell_transfer'])
                        ->orWhere(function ($q2) {
                            $q2->whereIn('transactions.type', ['purchase_transfer', 'sell_transfer'])
                                ->whereNull('transactions.van_id');
                        });
                })
            ...
            if ($stock_line->transaction_type == 'sell') {
                ...
                if (isset($stock_line->van_id)) {
                    ...
                    $stock_history_array[] = array_merge($temp_array, [
                        ...
                        'type' => 'sell',
                        'type_label' => __('sale.sale') . ' (' . $stock_line->van_name . ' 🚚)',
                ...
                if(isset($stock_line->van_id)){
                    ...
                    $stock_history_array[] = array_merge($temp_array, [
                        ...
                        'stock' => $this->roundQuantity($stock),
                        'type' => 'sell_transfer',
                        'type_label' => __('cashvan::stock.transfer_to_car') . ' (' .$stock_line->van_name. ' 🚚)',
                        'ref_no' => $stock_line->ref_no,
                        'stock_in_second_unit' => $this->roundQuantity($stock_in_second_unit),
                    ]);
                }else{
                ...
                if (isset($stock_line->van_id)) {
                    ...
                    $stock_history_array[] = array_merge($temp_array, [
                        ...
                        'type_label' => __('cashvan::stock.transfer_from_car')  . ' (' . $stock_line->van_name . ' 🚛)',
                        'ref_no' => $stock_line->ref_no,
                        'stock_in_second_unit' => $this->roundQuantity($stock_in_second_unit),
                    ]);
                }else{
