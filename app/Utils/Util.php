...
class Util
{
     ...
     * @return void
     */
    public function updateEditedSellLineCombo($lines, $location_id, $adjust_stock = true,$van_id=null)
    {
        if (empty($lines)) {
            return true;
        }
        ...
        foreach ($lines as $key => $line) {
            ...
            if ($difference != 0) {
                ...
                if ($adjust_stock) {
                    if($van_id){
                        VanStock::where('variation_id', $line['variation_id'])
    ...
    public function createUser($request)
    {
        $user_details = $request->only([
            ...
            'surname', 'first_name', 'last_name', 'email','categories','brands','accounts','defult_account',
        ...
        $user_details['is_sales_user'] = ! empty($request->input('is_sales_user')) ? $request->input('is_sales_user') : '0';
        ...
        $user_details['categories'] = json_encode($request->input('category', []));
        $user_details['brands'] = json_encode($request->input('brand', []));

        $user_details['accounts'] = json_encode($request->input('accounts', []));
        $user_details['defult_account'] = $request->input('defult_account');
