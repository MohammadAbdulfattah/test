...
class TransactionUtil extends Util
{
     ...
     * @return object
     */
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true, $van_id = null)
    {
        ...
        //dd(json_decode($input['applied_discounts_type1_ids'], true) );
        //dd($input['applied_discounts_type1_ids']);
        $discountIds = [];
        if (!empty($input['applied_discounts_type1_ids'])) {
            $decoded = json_decode($input['applied_discounts_type1_ids'], true);
            if (json_last_error() === JSON_ERROR_NONE && is_array($decoded) && count($decoded) > 0) {
                // Take first item only, and ensure it's numeric
                $firstId = intval($decoded[0]);
                if ($firstId > 0) {
                    $discountIds = $firstId;
                }
            }
        ...
        if ($van_id) {
            $transaction = Transaction::create([
                ...
                'van_id' => $van_id,
                'status_of_smart_root_discount_id' => $discountIds ?: null,
            ...
            // Save the applied smart root discount IDs if they exist
            // if (!empty($input['applied_smart_root_discount_ids'])) {
            //     $discountIds = json_decode($input['applied_smart_root_discount_ids']);
                
            //     if (is_array($discountIds) && !empty($discountIds)) {
            //         $transaction->statusOfSmartRootDiscounts()->sync($discountIds);
            //     }
            // }
     ...
     */
    public function createOrUpdateSellLines($transaction, $products, $location_id, $return_deleted = false, $status_before = null, $extra_line_parameters = [], $uf_data = true, $van_id = null)
    {
        $lines_formatted = [];
        $modifiers_array = [];
        $edit_ids = [0];
        ...
        foreach ($products as $product) {
            //dd($product['applied_discount_ids_type_two']);
            $multiplier = 1;
            ...
            if (! empty($product['transaction_sell_lines_id'])) {
                $edit_id_temp = $this->editSellLine($product, $location_id, $status_before, $multiplier, $uf_data, $van_id);
                ...
                $uf_unit_price = $uf_data ? $this->num_uf($product['unit_price'] ?? 0) : ($product['unit_price'] ?? 0);                $unit_price_before_discount = $uf_unit_price / $multiplier;
                ...
                $line = [
                    ...
                    'applied_discount_ids_type_three' => isset($product['applied_discount_ids']) 
                        ? json_decode($product['applied_discount_ids'], true) 
                        : null,
                    'applied_discount_ids_type_two' => isset($product['applied_discount_ids_type_two']) 
                        ? json_decode($product['applied_discount_ids_type_two'], true) 
                        : null,
                    'is_free' => $unit_price_before_discount === 0.0000 ? 1 : 0,
                ];
        ...
        if (! empty($edit_ids)) {
            ...
            $this->deleteSellLines($deleted_lines, $location_id, $adjust_qty, $van_id);
        }
     ...
     * @return bool
     */
    public function editSellLine($product, $location_id, $status_before, $multiplier = 1, $uf_data = true, $van_id = null)
    {
        ...
        if ($status_before != 'draft') {
            ...
            $this->adjustQuantity($location_id, $product['product_id'], $product['variation_id'], $difference, $van_id);
        }
     ...
     * @return bool
     */
    public function deleteSellLines($transaction_line_ids, $location_id, $adjust_qty = true, $van_id = null)
    {
        if (! empty($transaction_line_ids)) {
            ...
            foreach ($sell_lines as $line) {
                if ($adjust_qty) {
                    ...
                    $this->adjustQuantity($location_id, $line->product_id, $line->variation_id, $line->quantity, $van_id);
                }
     ...
     * @return bool
     */
    private function adjustQuantity($location_id, $product_id, $variation_id, $increment_qty, $van_id = null)
    {
        if ($increment_qty != 0) {
            ...
            if ($enable_stock == 1) {
                if ($van_id) {
                    ...
                    VanStock::where('variation_id', $variation_id)
                    ...
                    VariationLocationDetails::where('variation_id', $variation_id)
                        ...
                        ->update([
                            'qty_available' => DB::raw('qty_available + ' . $increment_qty),
                            'qty_in_vans' => DB::raw('qty_in_vans + ' . $increment_qty),
                        ]);
                } else {
                    //Adjust Quantity in variations location table
                    VariationLocationDetails::where('variation_id', $variation_id)
    ...
    public function getTotalPaymentsTransaction($business_id, $start_date = null, $end_date = null, $created_by = null)
    {
        $query = TransactionPayment::where('transaction_payments.business_id', $business_id)
            ->select(
        ...
        $transaction_payments = $query->first();

        $output['total_payments'] = $transaction_payments->total_payments;

        return $output;
    }

public function getTotalDue($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null)
{
    // Start building the query without group by
    ...
    $total_invoice = $result->total_invoice ?? 0;
    $invoice_received = $result->invoice_received ?? 0;
    $total_purchase = $result->total_purchase ?? 0;
    $purchase_paid = $result->purchase_paid ?? 0;
    $total_sell_return = $result->total_sell_return ?? 0;
    $sell_return_paid = $result->sell_return_paid ?? 0;
    $total_purchase_return = $result->total_purchase_return ?? 0;
    $purchase_return_received = $result->purchase_return_received ?? 0;
    $opening_balance = $result->opening_balance ?? 0;
    $opening_balance_paid = $result->opening_balance_paid ?? 0;
    $total_ledger_discount_sell = $result->total_ledger_discount_sell ?? 0;
    $total_ledger_discount_purchase = $result->total_ledger_discount_purchase ?? 0;

    // Calculate due
    $due = ($total_invoice - $invoice_received - $total_ledger_discount_sell)
         - ($total_purchase - $purchase_paid - $total_ledger_discount_purchase)
         - ($total_sell_return - $sell_return_paid)
         + ($total_purchase_return - $purchase_return_received)
         + ($opening_balance - $opening_balance_paid); // Opening balance is treated globally now

    return ['total_due' => round($due, 2)];
}
     ...
     * @return void
     */
    public function payAtOnce($parent_payment, $type,$locations=null)
    {
     ...
     * @return object
     */
    public function mapPurchaseSell($business, $transaction_lines, $mapping_type = 'purchase', $check_expiry = true, $purchase_line_id = null, $van_id = null,$for_van_update=false)
    {
     ...
     * @return array
     */
    public function deleteSale($business_id, $transaction_id, $van_id = null)
    {
    ...
    public function getListSells($business_id, $sale_type = 'sell')
    {
        $sells = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
            ...
            ->leftJoin('users as u', 'transactions.created_by', '=', 'u.id')
            ->leftJoin('users as d', 'transactions.driver_id', '=', 'd.id')
            ...
            ->select(
                ...
                DB::raw("CONCAT(COALESCE(u.surname, ''),' ',COALESCE(u.first_name, ''),' ',COALESCE(u.last_name,'')) as added_by"),
                DB::raw("CONCAT(COALESCE(d.surname, ''),' ',COALESCE(d.first_name, ''),' ',COALESCE(d.last_name,'')) as driver"),
     ...
     */
    public function __paymentQuery($contact_id, $start, $end = null, $location_id = null)
    {
        $business_id = request()->session()->get('user.business_id');
    ...
    public function payContact($request, $format_data = true)
    {
        ...
        if (! $is_reverse) {
            ...
            $excess_amount = $this->payAtOnce($parent_payment, $due_payment_type,$locations);
