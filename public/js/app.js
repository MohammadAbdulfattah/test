        ...
        $.ajax({
            ...
            success: function(result) {
                $('.pay_contact_due_modal')
                    .html(result)
                    .modal('show');
                    $('.pay_contact_due_modal').find('select.select2').select2({
                        width: '100%'  // you can customize options here
                    });
...
$(document).on('shown.bs.modal', '.pay_for_contact_modal', function() {
    if ($('#modal_customer_id').hasClass('select2-hidden-accessible')) {
        $('#modal_customer_id').select2('destroy');
    }

    $('#modal_customer_id').select2({
        dropdownParent: $('.pay_for_contact_modal'),
        ajax: {
            ...
            processResults: function(data) {
                // Add contact type to each result
                if (data && data.length) {
                    data.forEach(function(item) {
                        item.contact_type = item.type || 
                                         (item.supplier_business_name ? 'supplier' : 'customer');
                    });
                }
                return {
                    results: data || [],
    ...
    // Add hidden field for contact type
    if (!$('#contact_type_field').length) {
        $('.modal-body').append('<input type="hidden" id="contact_type_field" name="contact_type">');
    }
});

$(document).on('click', '#confirm_pay_contact', function(e) {
    e.preventDefault();
    
    let contactId = $('#modal_customer_id').val();
    if (!contactId) {
        alert("Please select a customer first");
        return;
    }

    let contactType = $('#contact_type_field').val();
    let dueType = contactType === 'supplier' ? 'purchase' : 'sell';
    let selectedLocations = $('#business_locations').val();

    let $btn = $(this);
    $btn.prop('disabled', true).html('<i class="fa fa-spinner fa-spin"></i> Processing...');

    let url = `/payments/pay-contact-due/${contactId}?type=${dueType}`;
    ...
    $('.pay_for_contact_modal').modal('hide');
    
    let $modalContainer = $('#dynamic-modal-container');
    if (!$modalContainer.length) {
        $modalContainer = $('<div id="dynamic-modal-container"></div>').appendTo('body');
    }
    
    $modalContainer.html(response);
    let $newModal = $modalContainer.find('.modal-dialog');
    
    // Create a wrapper div for better scrolling control
    let $modalWrapper = $('<div class="custom-modal-wrapper"></div>').appendTo('body');
    
    // Append the modal to the wrapper
    $newModal.appendTo($modalWrapper);
    
    // Style the wrapper to cover the whole screen
    $modalWrapper.css({
        'position': 'fixed',
        'top': '0',
        'left': '0',
        ...
        'height': '100%',
        'z-index': '1050',
        'overflow-y': 'auto' // Enable scrolling for the wrapper
    });
    ...
    // Add backdrop
    let $backdrop = $('<div class="custom-modal-backdrop"></div>').insertBefore($modalWrapper);
    $backdrop.css({
        'position': 'fixed',
        'top': '0',
        'left': '0',
    ...
    // Initialize plugins
    $newModal.find('.select2').select2({ width: '100%' });
    
    if ($newModal.find('input#paid_on').length) {
        $newModal.find('input#paid_on').datetimepicker({
            format: moment_date_format + ' ' + moment_time_format,
            ignoreReadonly: true
        });
    }
    ...
    // Ensure the modal is scrolled to the top initially
    ...
    $(document).on('change', '.pay_contact_due_modal #business_locations', function() {
        let contactId = $('input[name="contact_id"]').val();
        let dueType = $('input[name="due_payment_type"]').val();
        let selectedLocations = $('#business_locations').val();
        ...
        $.ajax({
            ...
            success: function (modalHtml) {
                const modal = $('.pay_contact_due_modal');
            
                // Parse the returned HTML into a temporary DOM
                const tempDOM = $('<div>').html(modalHtml);
                const oldSelect = modal.find('#business_locations');
                if (oldSelect.hasClass('select2-hidden-accessible')) {
                    oldSelect.select2('destroy');
                }
                // Replace the business location select
                const newSelect = tempDOM.find('#business_locations');
                modal.find('#business_locations').parent().html(newSelect.parent().html());
            
                // Replace the amount input
                const newAmount = tempDOM.find('input[name="amount"]');
                modal.find('input[name="amount"]').replaceWith(newAmount);
                ...
                modal.find('form#pay_contact_due_form').validate();
            
                // Optional: currency formatting
                __currency_convert_recursively(modal);
            },
            error: function() {
                alert('Failed to load updated modal content.');
            }
        });
    });
