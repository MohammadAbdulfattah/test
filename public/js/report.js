$(document).ready(function() {
    ...
    supplier_report_tbl = $('#supplier_report_tbl').DataTable({
        ...
        columns: [
            ...
            { data: 'days_count_from_last_payment', name: 'days_count_from_last_payment' },
...
function format_currency(amount) {
    var currency_symbol = __currency_symbol || 'د.ع'; // Make sure this is defined globally or passed from Blade
    var formatted = parseFloat(amount || 0).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,');
    return currency_symbol + ' ' + formatted;
}

function loadUserContactsDueReport(users) {
    if ($.fn.DataTable.isDataTable('#users_contacts_due_report_tbl')) {
        $('#users_contacts_due_report_tbl').DataTable().clear().destroy();
    }
    ...
    Object.keys(users).forEach(user_id => {
        columns.push({
            ...
            render: function(data, type, row) {
                return format_currency(data); // ✅ Format using currency function
            }
        });
    });
    ...
    columns.push({
        ...
        searchable: false,
        render: function(data, type, row) {
            return format_currency(data); // ✅ Format total too
        }
    });

    var table = $('#users_contacts_due_report_tbl').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            url: '/reports/user-contacts-due-report',
            data: function(d) {
                d.location_id = $('#cs_report_location_id').val();
                var dateRange = $('#scr_date_filter').data('daterangepicker');
                d.start_date = dateRange ? dateRange.startDate.format('YYYY-MM-DD') : '';
                d.end_date = dateRange ? dateRange.endDate.format('YYYY-MM-DD') : '';
                d.contact_id = $('#scr_contact_id').val();
                d.user_filter = $('#user_filter_id').val();
            }
        },
        columns: columns,
        columnDefs: [
            { targets: 0, width: '150px' } // Adjust width of first column
        ],
        footerCallback: function(row, data, start, end, display) {
            var api = this.api();
            var numUsers = Object.keys(users).length;

            // Update footer for user columns
            for (var i = 1; i <= numUsers; i++) {
                var sum = api.column(i, { search: 'applied' }).data()
                    .reduce(function(a, b) {
                        return a + parseFloat(b || 0);
                    }, 0);

                $(api.column(i).footer()).html(
                    '<strong>' + format_currency(sum) + '</strong>' // ✅ Format footer amount
                );
            }

            // Update total column
            var totalColumn = numUsers + 1;
            var totalSum = api.column(totalColumn, { search: 'applied' }).data()
                .reduce(function(a, b) {
                    return a + parseFloat(b || 0);
                }, 0);

            $(api.column(totalColumn).footer()).html(
                '<strong>' + format_currency(totalSum) + '</strong>' // ✅ Format total footer
            );
        },
        language: {
            emptyTable: "لا توجد بيانات متاحة",
            info: "عرض _START_ إلى _END_ من _TOTAL_ إدخال",
            infoEmpty: "عرض 0 إلى 0 من 0 إدخال",
            infoFiltered: "(تمت تصفيته من _MAX_ إدخال)",
    ...
    return table;
      ...
      $(document).ready(function () {
        loadUserContactsDueReport(all_users);

        $('#cs_report_location_id, #scr_contact_id, #user_filter_id').on('change', function () {
            loadUserContactsDueReport(all_users);
        });
    ...
    // // إعادة التحميل عند تغيير التاريخ
    ...
    if ($('#sr_date_filter').length == 1) {
        ...
        if ($('span#sr_total_sales').length > 0) {
         ...
         if ($('span#total_payments_transaction').length > 0) {
            getTotalPaymentsTransaction();
        }

        if ($('span#total_credit').length > 0) {
            getTotalDue();
        }
...
function getTotalPaymentsTransaction() {
    var start = $('input#sr_date_filter')
        .data('daterangepicker')
        .startDate.format('YYYY-MM-DD');
    var end = $('input#sr_date_filter')
    ...
    $('span#total_payments_transaction').html(__fa_awesome());

    $.ajax({
        method: 'GET',
        url: '/reports/total-payments-transaction',
        dataType: 'json',
        data: data_expense,
        success: function(data) {
            $('span#total_payments_transaction').html(__currency_trans_from_en(data.total_payments, true));
        },
    });
}
...
function getTotalDue() {
    ...
    $('span#total_credit').html(__fa_awesome());

    $.ajax({
        method: 'GET',
        url: '/reports/total-due',
        dataType: 'json',
        data: data_expense,
        success: function(data) {
            $('span#total_credit').html(__currency_trans_from_en(data.total_due, true));
        },
    });
}
