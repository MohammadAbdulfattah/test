...
$(document).ready(function() {
    ...
    $('table#pos_table tbody').on('change', 'input.pos_quantity', function() {
        ...
        // Skip if this is a free product row
        if (tr.hasClass('free-product-row')) return;
        ...
        //applySmartRootDiscount3();
        applySmartRootDiscount(tr);
    ...
    // When unit changes
    // $('table#pos_table tbody').on('change', 'select.sub_unit', function() {
    //     applySmartRootDiscount3();
    // });
    ...
    $('table#pos_table tbody').on('click', 'i.pos_remove_row', function() {
        var tr = $(this).closest('tr');
        if (!tr.hasClass('free-product-row')) {
            // We'll let applySmartRootDiscount3 handle removing associated free products
            setTimeout(applySmartRootDiscount3, 100);
        }
    });
    ...
    $('table#pos_table tbody').on('click', 'i.pos_remove_row', function() {
        ...
        // Remove all free products associated with this row
        //removeAssociatedFreeProducts(tr);
    ...
    $('button#pos-finalize').click(async function() {
        var customerId = $('select#customer_id').val();
        try {
            const response = await $.ajax({
                url: `/contacts/check/limit/pay/${customerId}`,
                method: 'GET',
                dataType: 'json',
                timeout: 3000
            });
            ...
            if (response && response.reason) {
                console.log('Template:', window.LANG_TEMPLATE);
                console.log('Reason:', response.reason);

                
                const errorMessage = window.LANG_TEMPLATE.replace('{reason}', response.reason);
                toastr.error(errorMessage);
                return false;
            }
        } catch (error) {
            console.error("Error checking customer limit:", error);
            // Optional: Show a warning but allow the sale to proceed
            toastr.warning("Could not verify customer limit. Proceeding with sale.");
        }
        //Check if product is present or not.
        if ($('table#pos_table tbody').find('.product_row').length <= 0) {
            toastr.warning(LANG.no_products_added);
    ...
    $('button.pos-express-finalize').click(async function(){
        var customerId = $('select#customer_id').val();
        try {
            const response = await $.ajax({
                url: `/contacts/check/limit/pay/${customerId}`,
...
// on click sub category in category drawer
    ...
    //     var tr = $(this).closest('tr');
    //     applySmartRootDiscount(tr);
    //     applySmartRootDiscount3(); // Also update cart-level discounts
    // });
...
function pos_product_row(variation_id = null, purchase_line_id = null, weighing_scale_barcode = null, quantity = null, unit = null, is_free = null) {
    ...
    // إذا كان المنتج هدية، نتجاوز طريقة الإضافة العادية ونضيف سطرًا جديدًا دائمًا
    if (is_free == 1) {
        add_via_ajax = true; // إضافة سطر جديد دائمًا للمنتجات الهدية
    } else if (variation_id != null && $('#item_addition_method').length) {
    ...
    if (item_addtn_method == 0) {
        ...
        // البحث في الصفوف الموجودة فقط للمنتجات غير الهدية
        $('#pos_table tbody')
            .find('tr:not(.free-product-row)') // نتجاهل الصفوف الخاصة بالمنتجات الهدية
            .each(function() {
                var row_v_id = $(this)
        ...
        $.ajax({
            ...
            success: function(result) {
                if (result.success) {
                    ...
                    $newRow.append('<input type="hidden" class="applied_discount_ids" name="products[' + product_row + '][applied_discount_ids]" value="[]">');

                    if (!$newRow.data('row-id')) {
                        $newRow.data('row-id', 'row-' + Date.now() + '-' + Math.floor(Math.random() * 1000));
                    }
                    ...
                    // For free products
                    if (is_free == 1) {
                        var $addedRow = $('table#pos_table tbody tr').last();
                        
                        // Mark as free product row
                        $addedRow.addClass('free-product-row');
                        
                        // Set price to 0
                        $addedRow.find('input.pos_unit_price_inc_tax').val(0);
                        $addedRow.find('input.pos_unit_price').val(0);
                        $addedRow.find('input.pos_line_total').val(0);
                        $addedRow.find('select.sub_unit').val(unit);
                        // Disable price editing
                        $addedRow.find('.pos_unit_price_inc_tax, .pos_unit_price').prop('readonly', true);
                        
                        // Disable quantity editing
                        $addedRow.find('.pos_quantity').prop('readonly', true).css('pointer-events', 'none');
                        $addedRow.find('.quantity_buttons button').prop('disabled', true).css('pointer-events', 'none');
                        
                        // Disable unit/subunit selection
                        $addedRow.find('select.sub_unit').prop('disabled', true).css('pointer-events', 'none');
                        
                        // Disable remove button
                        $addedRow.find('.pos_remove').prop('disabled', true).css('pointer-events', 'none');
                        
                        // Update display
                        $addedRow.find('span.pos_line_total_text').text('0');
                        
                        // Also prevent any keyboard input
                        $addedRow.find('.pos_quantity').on('keydown', function(e) {
                            e.preventDefault();
                            return false;
                        });
                        $addedRow.find('td:first').prepend(
                            '<span class="label label-success" style="margin-right:5px;">FREE</span>'
                        );
                        //$addedRow.append('<input type="hidden" class="is_free_product" name="products[' + product_row + '][is_free]" value="1">');
                    ...
                    applySmartRootDiscount(this_row);
                    applySmartRootDiscount3(this_row)
...
function applySmartRootDiscount(row) {
    if (row.hasClass('free-product-row')) return;
    var variation_id = row.find('.row_variation_id').val();
    var unit_id = row.find('select.sub_unit').val() || row.find('.unit_id').val();
    var quantity = __read_number(row.find('.pos_quantity'));
    var pos_unit_price_inc_tax = __read_number(row.find('.pos_unit_price_inc_tax'));
    var applied_discount_ids = [];
    
    // Store original price in data attribute
    if (!row.find('.pos_unit_price_inc_tax').data('original-price')) {
        row.find('.pos_unit_price_inc_tax').data('original-price', pos_unit_price_inc_tax);
    }
    var original_price = row.find('.pos_unit_price_inc_tax').data('original-price');
    
    $.ajax({
        url: '/check-smart-root-discount2',
        method: 'POST',
        data: {
            variation_id: variation_id,
            quantity: quantity,
            unit_id: unit_id,
            pos_unit_price_inc_tax: original_price,
            applied_discount_ids: applied_discount_ids
        },
        success: function(response) {
            if (response.success) {
                var discountDisplay = row.find('.discount-names');
                if (discountDisplay.length === 0) {
                    discountDisplay = $('<span class="discount-names" style="font-size:11px; color:#28a745; display:block;"></span>');
                    row.find('td:eq(1)').append(discountDisplay);
                }
                
                if (response.has_discount && response.applied_discounts && response.applied_discounts.length > 0) {
                    var lastDiscount = response.applied_discounts[0];
                    discountDisplay.text(__messages3.discounts_type_three + ':' + lastDiscount.smart_root_discount_name);
                    __write_number(row.find('.pos_unit_price_inc_tax'), response.discounted_unit_price);
                    
                    if (response.applied_discounts && response.applied_discounts.length > 0) {
                        __write_number(row.find('input.row_discount_amount'), response.applied_discounts[0].condition_discount_amount);
                        var discountTypeSelect = row.find('select.row_discount_type');
                        discountTypeSelect.val(response.applied_discounts[0].discount_type).trigger('change');
                    }
                } else {
                    // Clear all discount-related fields when no discount applies
                    discountDisplay.text('');
                    __write_number(row.find('.pos_unit_price_inc_tax'), response.original_unit_price);
                    __write_number(row.find('input.row_discount_amount'), 0);
                    row.find('select.row_discount_type').val('fixed').trigger('change');
                    row.find('.applied_discount_ids').val('[]');
                }
                
                // Update line total
                var new_qty = __read_number(row.find('.pos_quantity'));
                var new_line_total = __read_number(row.find('.pos_unit_price_inc_tax')) * new_qty;
                __write_number(row.find('.pos_line_total'), new_line_total);
                row.find('.pos_line_total_text').text(__currency_trans_from_en(new_line_total, true));

                // Update discount IDs if applicable
                if (response.applied_discounts_ids && response.applied_discounts_ids.length > 0) {
                    var discountIdsString = JSON.stringify(response.applied_discounts_ids);
                    var discountIdsElement = row.find('.applied_discount_ids');
                    
                    if (discountIdsElement.length === 0) {
                        var productRow = row.find('input[name^="products["]').first();
                        var nameAttr = productRow.attr('name').match(/products\[(\d+)\]/);
                        if (nameAttr && nameAttr[1]) {
                            row.append('<input type="hidden" class="applied_discount_ids" name="products[' + nameAttr[1] + '][applied_discount_ids]" value="' + discountIdsString + '">');
                        }
                    } else {
                        discountIdsElement.val(discountIdsString);
                    }
                } else {
                    row.find('.applied_discount_ids').val('[]');
                }
        ...
        error: function(xhr) {
            console.error('Failed to check smart root discount', xhr.responseText);
            toastr.error('Failed to apply discount. Please try again.');
        }
    });
}
...
function applySmartRootDiscount3() {
    if (isApplyingDiscount) return;
    isApplyingDiscount = true;

    $('tr.free-product-row').remove();

    var cart_items = [];
    
    $('table#pos_table tbody tr.product_row:not(.free-product-row)').each(function() {
        var tr = $(this);
         if (tr.hasClass('free-product-row') || tr.data('is_free') === 1) {
        return true;
    }
        var variation_id = tr.find('.row_variation_id').val();
        if (variation_id) {
            cart_items.push({
                product_id: tr.find('.product_id').val(),
                variation_id: variation_id,
                unit_id: tr.find('select.sub_unit').val() || tr.find('.unit_id').val(),
                quantity: __read_number(tr.find('.pos_quantity')),
                unit_price: 0,
    ...
    $.ajax({
        url: '/check-smart-root-discount3',
        method: 'POST',
        data: {
        ...
        success: function(response) {
            if (response.success) {
                ...
                var cartDiscountDisplay = $('#cart-discounts-display');
                if (!cartDiscountDisplay.length) {
                    cartDiscountDisplay = $('<div id="cart-discounts-display" style="font-size:12px; color:#28a745; padding:5px;"></div>');
                    $('.pos_form_totals').prepend(cartDiscountDisplay);
                }

                if (response.has_discount && response.applied_discounts_info?.length) {
                    var discountNames = response.applied_discounts_info.map(d => d.name).join(', ');
                    cartDiscountDisplay.text(__messages2.discounts_type_two + ': ' + discountNames);

                    // Add free products (if any)
                    if (response.products_to_add) {
                        const productsToAdd = Array.isArray(response.products_to_add) 
                            ? response.products_to_add 
                            : [response.products_to_add];
                        
                        productsToAdd.forEach(function(product) {
                            const existingFreeProduct = $(`tr.free-product-row [data-variation-id="${product.variation_id}"]`).closest('tr');
                            if (!existingFreeProduct.length) {
                                console.log('Adding free product:', product.variation_id);
                                console.log('unit',response.products_to_add.unit_name);
                                pos_product_row(product.variation_id, null, null, product.quantity, product.unit_id,1);
                            }
                        });
                    }
                    console.log('three_discount',response.applied_discounts_ids);
                    if (response.applied_discounts_ids && response.applied_discounts_ids.length > 0) {
                        var discountIdsString = JSON.stringify(response.applied_discounts_ids);
                        
                        // Find all product rows and add the discount IDs
                        $('table#pos_table tbody tr.product_row:not(.free-product-row)').each(function() {
                            var row = $(this);
                            var discountIdsElement = row.find('.applied_discount_ids_type_two');
                            
                            if (discountIdsElement.length === 0) {
                                var productRow = row.find('input[name^="products["]').first();
                                var nameAttr = productRow.attr('name').match(/products\[(\d+)\]/);
                                if (nameAttr && nameAttr[1]) {
                                    row.append('<input type="hidden" class="applied_discount_ids_type_two" name="products[' + nameAttr[1] + '][applied_discount_ids_type_two]" value="' + discountIdsString + '">');                          }
                            } else {
                                discountIdsElement.val(discountIdsString);
                            }
                        });
                    } else {
                        $('.applied_discount_ids_type_two').remove();
                    }

                 

                } else {
                    cartDiscountDisplay.text('');
                    $('#applied_discount_ids').remove();
                }
            }
            isApplyingDiscount = false;
        ...
        error: function(xhr) {
            console.error('Discount error:', xhr.responseText);
...
// Remove all free products associated with a parent row
function removeAssociatedFreeProducts(parentRow) {
    // Generate unique identifier for this row
     var rowId = parentRow.data('row-id');
    
    if (!rowId) {
        // If no row-id exists, generate one (this shouldn't happen as we set it when adding)
        rowId = 'row-' + Date.now() + '-' + Math.floor(Math.random() * 1000);
        parentRow.data('row-id', rowId);
    }
    
    // Remove all free products that reference this parent
    $('tr.free-product-row').each(function() {
        var $freeRow = $(this);
        if ($freeRow.data('parent-id') === rowId) {
            $freeRow.remove();
        }
    });
}

// Modified helper function to add uneditable product rows
function addUneditableProductRow(variation_id, quantity, unit_id, discount_id, parentId) {
    var product_row = $('input#product_row_count').val();
    var location_id = $('input#location_id').val();
    ...
    $.ajax({
        ...
        data: {
            product_row: product_row,
            is_direct_sell: $('input[name="is_direct_sale"]').val() == 1,
            quantity: quantity,
            is_uneditable: true
        },
        success: function(result) {
            if (result.success) {
                ...
                // Replace quantity input with plain text
                var qtyElement = $newRow.find('input.pos_quantity');
                var qtyValue = qtyElement.val();
                qtyElement.replaceWith('<span class="non-editable-qty">' + qtyValue + '</span>');
                console.log(qtyValue);
                // Disable all other editing elements
                $newRow.find('input, select').prop('disabled', true)
                       .css('background-color', '#f5f5f5');
                $newRow.find('.pos_remove_row').remove();
                //$newRow.find('input.pos_unit_price_inc_tax').val(0);
                
                // Set prices to 0
                __write_number($newRow.find('input.pos_unit_price'), 0);
                __write_number($newRow.find('input.pos_unit_price_inc_tax'), 0);
                __write_number($newRow.find('input.pos_line_total'), 0);
                $newRow.find('span.pos_line_total_text').text(__currency_trans_from_en(0, true));
                
                // Set unit if specified and disable the dropdown
                if (unit_id) {
                    var unitSelect = $newRow.find('select.sub_unit');
                    unitSelect.val(unit_id);
                    unitSelect.prop('disabled', true);
                    unitSelect.css('background-color', '#f5f5f5');
                    unitSelect.css('appearance', 'none');
                    unitSelect.css('-webkit-appearance', 'none');
                    unitSelect.css('-moz-appearance', 'none');
                }
                
                // Add visual indicator
                $newRow.css('background-color', '#f0fff0')
                       .find('td:first')
                       .prepend('<i class="fa fa-gift text-success" title="Free Product"></i> ');
                
                // Add to table
                $('table#pos_table tbody').append($newRow);
                $('input#product_row_count').val(parseInt(product_row) + 1);
            }
        }
    });
...
function calculate_billing_details(price_total) {
   ...
   function pos_discount(total_amount) {
        ...
        $.ajax({
            ...
            //async: false,
            success: function(response) {
                var cartDiscountDisplayTypeone = $('#cart-discounts-display-type-one');
                if (!cartDiscountDisplayTypeone.length) {
                    cartDiscountDisplayTypeone = $('<div id="cart-discounts-display-type-one" style="font-size:12px; color:#28a745; padding:5px;"></div>');
                    $('.pos_form_totals').prepend(cartDiscountDisplayTypeone);
                }

                if (response.has_discount && response.applied_discount) {
                    cartDiscountDisplayTypeone.text(__messages.discounts_type_one + ': ' + response.applied_discount.smart_root_discount_name);

                    discount = response.total_discount;

                    var discountType1Element = $('#applied_discounts_type1_ids');
                    var discountId = response.applied_discount.id;
                 
                    if (discountType1Element.length === 0) {
                        $('#applied_discounts_type1_ids').remove();
                        $('form').append(
                            '<input type="hidden" id="applied_discounts_type1_ids" name="applied_discounts_type1_ids" value=\'' + JSON.stringify([discountId]) + '\'>'
                        );
                    } else {
                        discountType1Element.val(JSON.stringify([discountId]));
                    }
                    __write_number($('#discount_amount'), response.applied_discount.discount_amount);
                    $('#discount_type').val(response.applied_discount.discount_type).trigger('change');
                    console.log('Applied Discount ID:', discountId);

                } else {
                    cartDiscountDisplayTypeone.text('');
                    // var calculation_type = $('#discount_type').val();
                    // var calculation_amount = __read_number($('#discount_amount'));
                    // discount = __calculate_amount(calculation_type, calculation_amount, total_amount);
                    $('#applied_discounts_type1_ids').val('[]');
                }
            },
            error: function(xhr, status, error) {
                console.error("Error fetching smart discount:", xhr.responseText);
                // var calculation_type = $('#discount_type').val();
                // var calculation_amount = __read_number($('#discount_amount'));
                // discount = __calculate_amount(calculation_type, calculation_amount, total_amount);
                $('#applied_discounts_type1_ids').val('[]');
