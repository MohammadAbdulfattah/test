<div class="row">
	...
	@if(!empty($show_date))
	<div class="{{$col_class}}  @if(!auth()->user()->can('payment_date.access')) hide @endif">
		<div class="form-group">
    ...
    @php
	$user = auth()->user();
	$_defult_account = $user->defult_account ? $user->defult_account : $accounts;
		@endphp


    {!! Form::select("payment[$row_index][account_id]", $accounts, $_defult_account, ['class' => 'form-control col-md-12 account-dropdown', 'id' => !$readonly ? "method_$row_index" : "method_advance_$row_index", 'style' => 'width:100%;', 'disabled' => $readonly]); !!}

    @if($readonly)
      {!! Form::hidden("payment[$row_index][account_id]", $payment_line['account_id'], ['class' => 'account-dropdown', 'id' => "method_$row_index"]); !!}
    @endif
</div>
			</div>
		</div>
