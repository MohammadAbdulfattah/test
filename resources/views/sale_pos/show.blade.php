        ...
        <div class="table-responsive">
          <table class="table bg-gray">
            ...
            <tr>
              ...
              <td>
                @php
                  $sell_return = !empty($sell->return_parent) ? $sell->return_parent->final_total : 0;
                  $sell_return = (string) $sell_return;
                @endphp
                <span class="display_currency pull-right" data-currency_symbol="true" >{{ $sell_return }}</span></td>
            </tr>
            <tr>
              <th>{{ __('lang_v1.total_with_sell_return') }}:</th>
              <td></td>
              <td>
                @php
                  $sell_return = !empty($sell->return_parent) ? $sell->return_parent->final_total : 0;
                  $sell_return = (string) $sell_return;
                @endphp
                <span class="display_currency pull-right" data-currency_symbol="true" >{{ $sell->final_total - $sell_return }}</span></td>
            </tr>
            @if($sell->type != 'sales_order')
            <tr>
