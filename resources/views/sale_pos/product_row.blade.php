...
@if($product->is_free == 1)
    <tr class="product_row free-product-row" data-is_free="1">
@else
...
<br>
	...
	<td>
        ...
        <span class="input-group-btn">
            <button type="button" class="btn btn-default btn-flat quantity-down" 
                @if(isset($product->is_free) && $product->is_free == 1) disabled @endif>
        ...
        <input type="text" data-min="1" style="width: auto"
            ...
            @if(isset($product->is_free) && $product->is_free == 1) 
                disabled 
            @endif
    ...
    @endforeach
		...
		@else
			{{$product->unit}}
			 <input type="hidden" class="unit_id" value="{{$product->unit_id}}">
		@endif
		...
		@if($product->product_type == 'combo'&& !empty($product->combo_products))
			...
			@foreach($product->combo_products as $k => $combo_product)
				...
				<input type="hidden" 
					...
					<input type="hidden" class="applied_discount_ids" name="products[__index__][applied_discount_ids]" value="">
                    <input type="hidden" name="applied_discounts_type1_ids" id="applied_smart_root_discount_ids" value="[]">
					<input type="hidden" class="applied_discount_ids_type_two" name="products[__index__][applied_discount_ids_type_two]" value="">
					@if(isset($action) && $action == 'edit')
						<input type="hidden" 
	...
	<td class="text-center v-center">
		@if($product->is_free == 1)
			<i class="fa fa-times text-muted cursor-pointer" style="pointer-events: none; opacity: 0.5;" title="@lang('lang_v1.free_product_cannot_delete')"></i>
		@else
