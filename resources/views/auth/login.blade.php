...
@section('content')
@if ($errors->has('shift'))
    <div class="alert alert-danger" id="shift-error-alert">
        <ul>
            <li>{{ $errors->first("shift") }}</li>
        </ul>
    </div>

    <script>
        setTimeout(function() {
            const alert = document.getElementById('shift-error-alert');
            if (alert) {
                alert.style.transition = "opacity 0.5s ease";
                alert.style.opacity = 0;
                setTimeout(() => alert.remove(), 500); // remove it from DOM after fade out
            }
        }, 3000); // 3 seconds
    </script>
@endif
    ...
    @endif
        ...
        <div class="col-md-4  " style="min-height: 100vh; padding-top: 100px;">
            <div
                class="tw-p-5 md:tw-p-6 tw-mb-4 tw-transition-all tw-rounded-md tw-mt-6 "  style="min-height: 450px;">
                <div class="tw-flex tw-flex-col tw-gap-4 tw-dw-rounded-box tw-dw-p-6 tw-dw-max-w-md">
                    ...
                    <div class="tw-flex tw-items-center tw-flex-col">
                        ...
                        <h2 class="tw-text-sm tw-font-medium tw-text-gray-500">
                            @lang('lang_v1.login_to_your') {{ config('app.name', 'ROOT') }}
                        </h2>
                    </div>
                    ...
                    <form method="POST" action="{{ route('login') }}" id="login-form">
                        ...
                        <div class="form-group has-feedback {{ $errors->has('username') ? ' has-error' : '' }}">
                            <label class="tw-dw-form-control">
                                ...
                                <input
                                    class="tw-border tw-border-[#D1D5DA] tw-outline-none tw-h-12 tw-bg-transparent tw-rounded-md tw-px-3 tw-font-medium tw-text-black placeholder:tw-text-gray-500 placeholder:tw-font-medium"
