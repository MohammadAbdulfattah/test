...
<section class="content">
   @php
    $enabled_user_modules = (array)(!empty(session('business.enabled_user_modules')) ? session('business.enabled_user_modules') : []);
    @endphp
{!! Form::open(['url' => action([\App\Http\Controllers\ManageUserController::class, 'store']), 'method' => 'post', 'id' => 'user_add_form' ]) !!}
  <div class="row">
    <div class="col-md-12">
  @component('components.widget')
      <div class="col-md-2 @if(!in_array('enable_surname', $enabled_user_modules)) hide @endif">
      ...
      <div class="col-md-2 @if(!in_array('enable_is_active', $enabled_user_modules)) hide @endif">
        <div class="form-group">
          <div class="checkbox">
            <br/>
            <label>
      ...
      <div class="col-md-3 @if(!in_array('enable_service_staff_pin', $enabled_user_modules)) hide @endif">
      ...
      <div class="col-md-2">
        <div class="form-group">
            <div class="checkbox">
                <br/>
                <label>
                    {!! Form::checkbox('is_sales_user', 1, isset($user) ? $user->is_sales_user : false, ['class' => 'input-icheck']) !!}
                    {{ __('lang_v1.is_sales_user') }}
                </label>
  ...
  <div class="col-md-12">
    @component('components.widget', ['title' => __('lang_v1.roles_and_permissions')])
      ...
      <div class="clearfix"></div>
      <div class="col-md-3 @if(!in_array('enable_user_location', $enabled_user_modules)) hide @endif">
          <h4>@lang( 'role.access_locations' ) @show_tooltip(__('tooltip.access_locations_permission'))</h4>
        </div>
        <div class="col-md-9 @if(!in_array('enable_user_location', $enabled_user_modules)) hide @endif">
          <div class="col-md-12">
            <div class="checkbox">
                <label>
  ...
  <div class="col-md-12">
    @component('components.widget', ['title' => __('lang_v1.roles_of_show_products')])
      ...
      <div class="col-md-6">
        <div class="form-group">
          {!! Form::label('category', __( 'category.categories' ) . ':*') !!} @show_tooltip(__('lang_v1.products_by_categories'))
            {!! Form::select('category[]', $categories, null, ['class' => 'form-control select2' , 'multiple' , 'id' => 'categories_select']); !!}
        </div>
      </div>

      <div class="col-md-6">
        <div class="form-group">
          {!! Form::label('brand', __( 'brand.brands' ) . ':*') !!} @show_tooltip(__('lang_v1.products_by_brands'))
            {!! Form::select('brand[]', $brands, null, ['class' => 'form-control select2' , 'multiple' , 'id' => 'brands_select']); !!}
        </div>
      </div>
  ...
  <div class="col-md-12">
    @component('components.widget', ['title' => __('lang_v1.show_accounts_permissions')])
      ...
      <div class="col-md-6">
          <div class="form-group">
              {!! Form::label('accounts', __( 'lang_v1.accounts' ) . ':*') !!} 
              @show_tooltip(__('lang_v1.show_accounts_by_permissions'))
              {!! Form::select('accounts[]', $accounts, null, [
      ...
      <div class="col-md-6">
          <div class="form-group">
              {!! Form::label('defult_account', __( 'lang_v1.defult_account' ) . ':*') !!} 
              @show_tooltip(__('lang_v1.show_the_defult_acconut'))
              {!! Form::select('defult_account', [null => 'لا أحد'] + $accounts->toArray(), null, [
                  'class' => 'form-control select2', 
                  'id' => 'defult_account_select'
              ]) !!}
          </div>
      </div>
    @endcomponent
  ...
  <div class="col-md-12 @if(!in_array('enable_sales_section', $enabled_user_modules)) hide @endif">
  ...
  @if(in_array('enable_more_info_section', $enabled_user_modules)) 
...
<script type="text/javascript">
  ...
  $('#categories_select').on('change', function() {
      if ($(this).val() && $(this).val().length > 0) {
          // Clear and disable brands select
          $('#brands_select').val(null).trigger('change');
          $('#brands_select').prop('disabled', true);
      } else {
          // Enable brands select if no categories are selected
          $('#brands_select').prop('disabled', false);
      }
  });
  ...
  $('#brands_select').on('change', function() {
      if ($(this).val() && $(this).val().length > 0) {
          // Clear and disable categories select
          $('#categories_select').val(null).trigger('change');
          $('#categories_select').prop('disabled', true);
      } else {
          // Enable categories select if no brands are selected
