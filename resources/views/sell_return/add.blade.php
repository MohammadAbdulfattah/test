	...
	<div class="box box-solid">
		<div class="box-body">
			<div class="row">
				...
				<div class="col-sm-3  @if(!auth()->user()->can('date.access')) hide @endif">
					<div class="form-group">
					...
					<div class="col-sm-3">
						<div class="form-group">
							<label class="checkbox-inline">
								<input type="checkbox" 
									id="move_to_stock_adjustment" 
									name="move_to_stock_adjustment"
									value="1">
								@lang('messages.move_quantity_to_stock_adjustment_list')
							</label>
						</div>
					</div>
					@endif
					<!-- Confirmation Modal -->
					<div class="modal fade" id="confirmMoveModal" tabindex="-1" role="dialog" aria-labelledby="confirmMoveModalLabel">
						<div class="modal-dialog" role="document">
							<div class="modal-content">
								<div class="modal-header">
									<h4 class="modal-title" id="confirmMoveModalLabel">@lang('messages.confirmation')</h4>
									<button type="button" class="close" data-dismiss="modal" aria-label="Close">
										<span aria-hidden="true">&times;</span>
									</button>
								</div>
								<div class="modal-body">
									@lang('messages.are_you_sure_move_to_stock_adjustment')
								</div>
								<div class="modal-footer">
									<button type="button" class="btn btn-default" data-dismiss="modal">@lang('messages.cancel')</button>
									<button type="button" class="btn btn-primary" id="confirmMoveButton">@lang('messages.confirm')</button>
								</div>
							</div>
						</div>
					</div>
...
$(document).ready(function() {
        ...
        if ($('#move_to_stock_adjustment').is(':checked')) {
            if (confirm("@lang('messages.are_you_sure_move_to_stock_adjustment')")) {
                $.ajax({
                    url: '/add_stock_adjustment_line/' + {{ $sell->id }},
                    type: 'GET',
                    success: function(response) {
                        if (response.success) {
                            toastr.success(response.msg);
                            // Redirect to the stock adjustment view
            ...
            // Submit normally if checkbox not checked
    ...
    // When the modal is shown, store the ID
    $('#confirmMoveModal').on('show.bs.modal', function (event) {
        var button = $(event.relatedTarget); // Button that triggered the modal
        itemId = button.data('id'); // Extract ID from data-id attribute
    });
