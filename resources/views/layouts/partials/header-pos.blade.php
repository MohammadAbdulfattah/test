    ...
    {{-- <div class="row">
        ...
        <div class="col-md-6">
            ...
            @php
                $userId = auth()->id();
            @endphp
            @can('show_smart_root_discounts_user')
                <button type="button" id="smart_root_discounts" title="{{ __('lang_v1.root_smart_discounts') }}"
                    ...
                    data-container=".smart_root_discounts_modal"
                    data-href="{{ action([\App\Http\Controllers\DiscountController::class, 'getSmartRootDiscountByUser'], ['id' => $userId]) }}">

                    <strong class="!tw-m-3">
                        <i class="fas fa-percent tw-fa-lg tw-text-[#00935F] !tw-text-sm" aria-hidden="true"></i>
                        <span class="tw-inline md:tw-hidden">{{ __('lang_v1.root_smart_discounts') }}</span>
                    </strong>
                </button>
            @endcan

           @can('pay_for_contact')
            <button type="button" id="pay_for_contact" title="{{ __('lang_v1.pay_for_contact') }}"
                class="tw-shadow-[rgba(17,_17,_26,_0.1)_0px_0px_16px] tw-bg-white hover:tw-bg-white/60 tw-cursor-pointer tw-border-2 tw-flex tw-items-center tw-justify-center tw-rounded-md md:tw-w-8 tw-w-auto tw-h-8 tw-text-gray-600 btn-modal pull-right"
                data-container=".pay_for_contact_modal"
                data-href="{{ action([\App\Http\Controllers\SellPosController::class, 'selectContactToPay']) }}">

                <strong class="!tw-m-3">
                    <!-- تغيير الأيقونة إلى أيقونة الدفع -->
                    <i class="fas fa-money-bill-wave tw-text-[#00935F] !tw-text-sm" aria-hidden="true"></i>
                    <span class="tw-inline md:tw-hidden">{{ __('lang_v1.pay_for_contact') }}</span>
                </strong>
            </button>
        @endcan
